...
def solution1():
    ...
    #mathy way
    print (gaussian_addition(999, 5) + gaussian_addition(999, 3) - gaussian_addition(999, 15))

def solution2():
    print sum(filter(lambda x: x % 2 == 0, fib_list(4000000)))
...
def solution6():
    ...
    for x in xrange(101):
        total += pow(x, 2)
    print pow(gaussian_addition(100), 2) - total
...
def solution8():
    ...
    while number > 0:
        test = digitalproduct(number % 100000)
        number /= 10
        if test > maxnum:
            maxnum = test
    print maxnum
...
def solution11():
    print adjacency_search([[8, 02, 22, 97, 38, 15, 00, 40, 00, 75, 04, 05, 07, 78, 52, 12, 50, 77, 91, 8],
...
def solution12():
    ...
    while maxnum <= 500:
        ...
        odd = newodd
        even = neweven
        test = num_divisors(odd) * num_divisors(even / 2)
        if test > maxnum:
            maxnum = test
    return gaussian_addition(min(even, odd))

def solution14():
    maxnum = 0
    bestnum = 1
    for x in xrange(500001, 999999, 2):
        test = collatz(x)
        if test > maxnum:
            maxnum = test
            bestnum = x
...
def gaussian_addition(upper_limit, increment=1):
    n = upper_limit/increment
    return increment * (n * (n + 1)) / 2

def fib_list(upper_limit):
    L=[1, 1]
    while sum(L[-2:]) <= upper_limit:
        L.append(sum(L[-2:]))
    return L
...
def sieve(upper_limit):
    if upper_limit < 2:
        return []
    L = range(2, upper_limit+1)
    upper_limit = sqrt(upper_limit)
    S = []
    while L[0] <= upper_limit:
        S.append(L[0])
        L = filter(lambda x: x % L[0] != 0, L)
    S.extend(L)
    return S
...
def factor(number):
    if number <= 3:
        return [number]
    P = []
    for x in sieve(int(sqrt(number))):
        while number % x == 0:
            P.append(x)
            number /= x
        if number == 1:
...
def num_divisors(number):
    ...
    divisors = 1
    while P != []:
        divisors *= (1 + P.count(P[0]))
        P = filter(lambda x: x != P[0], P)
    return divisors

def lcm(L):
    P = []
    while L != []:
        P.extend(factor(filter(lambda x: log(x, L[0]) % 1 == 0, L)[-1]))
        L = filter(lambda x: x % L[0] != 0, L)
...
def adjacency_search(grid):
    maxnum = 1
    for x in xrange(len(grid) - 4):
        for y in xrange(len(grid[0]) - 4):
            if x > 2:
                test = grid[x][y] * grid[x - 1][y + 1] * grid[x - 2][y + 2] * grid[x - 3][y + 3]
                if test > maxnum:
                    maxnum = test
            test = grid[x][y] * grid[x][y + 1] * grid[x][y + 2] * grid[x][y + 3]
            if test > maxnum:
                maxnum = test
            test = grid[x][y] * grid[x + 1][y + 1] * grid[x + 2][y + 2] * grid[x + 3][y + 3]
            if test > maxnum:
                maxnum = test
            test = grid[x][y] * grid[x + 1][y] * grid[x + 2][y] * grid[x + 3][y]
            if test > maxnum:
                maxnum = test
    return maxnum
...
def find_route(triangle):
    row = len(triangle) - 1
    while row > 0:
        for x in xrange(len(triangle[row - 1])):
            triangle[row - 1][x] += max(triangle[row][x], triangle[row][x + 1])
        row -= 1
    return triangle[0][0]
